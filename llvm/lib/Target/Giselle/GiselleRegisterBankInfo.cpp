...
GiselleRegisterBankInfo::getInstrMapping(const MachineInstr &MI) const {
  ...
  const ValueMapping *GPR16ValueMapping =
      &Giselle::ValueMappings[Giselle::GPRB16Idx];
  const ValueMapping *GPR32ValueMapping =
      &Giselle::ValueMappings[Giselle::GPRB32Idx];
...
const RegisterBank &
GiselleRegisterBankInfo::getRegBankFromRegClass(const TargetRegisterClass &RC,
                                              LLT Ty) const {
  switch (RC.getID()) {
  default:
    llvm_unreachable("Register class not supported");
  case Giselle::GPR32RegClassID:
    return getRegBank(Giselle::GPRBRegBankID);
  }
}
